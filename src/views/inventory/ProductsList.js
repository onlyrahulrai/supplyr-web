...
class SubcategorySelector extends React.Component {
  constructor(props){
    ...
    this.state = {
      ...
      isSubmitting: false,
      searchBarValue: '',
      displayClearSelectionButton: this.props.displayClearSelectionButton ?? false,
    }
  }
  ...
  handleOnConfirm = e => {
    if (this.props.onConfirmPromise) {
      ...
      let promise = this.props.onConfirmPromise(this.state.selectedSubCategories)
      promise.then(() => {
  ...
  render() {
    const displayedCategories = this.state.displayedSubCategories?.groupBy('category') || {}
    return (
      <div className={this.props.className + " d-inline-block"}>
        <ButtonGroup >
          <Button.Ripple disabled={this.props.disabled} outline={this.state.selectedSubCategories.length===0} color="dark" onClick={this.onCategoryButtonClick}>
            {this.props.buttonIcon ?? <Folder size={14} />}
            <span className ="ml-50">{this.props.buttonLabel ?? 'Categories'}</span>
          </Button.Ripple>
          {this.state.displayClearSelectionButton && this.state.selectedSubCategories.length!==0 &&
          <Button color="dark" outline className="p-50" onClick={e => this.onClearAllSubCategories()}>
            <X size={14} />
          </Button>
          }
        </ButtonGroup>
        <Modal isOpen={this.state.categoryModalOpen} toggle={this.toggleCategoryModal} className="modal-dialog-centered">
          <ModalHeader toggle={this.toggleCategoryModal} className="bg-dark">{this.props.modalTitle}</ModalHeader>
          <ModalBody>
            <SimpleInputField 
              placeholder="search..."
                      ...
                      return (
                        <div key={sc.id}>
                          <Checkbox
                            ...
                            defaultChecked={this.state.selectedSubCategories.includes(sc.id)}
                            onChange={e => {
                              const checked = e.target.checked
                              const existingSelected = this.state.selectedSubCategories
                              if (checked){
                                !existingSelected.find(i => i === sc.id) && this.setState({selectedSubCategories: [...existingSelected, sc.id]})
                              }
                              else {
                                existingSelected.find(i => i === sc.id) && this.setState({selectedSubCategories: existingSelected.filter(i => i !== sc.id)})
                              }
                            }}
                          />
                        </div>
          ...
          <ModalFooter>
            ...
            > {this.state.isSubmitting && <Spinner color="light" size="sm"  className="reload-spinner mr-1" />}
...
class UsersList extends React.Component {
  ...
  async fetchProducts(options) {
    ...
    try{
      ...
      !cached && this.setState({requestsCache: {...this.state.requestsCache, [queryParams]:response}})
    }
    catch(e) {
  ...
  bulkUpdate = (operation, data) => {
    ...
    return apiClient.post('/inventory/products/bulk-update/', {
      product_ids,
      operation,
      data,
    }).then((response) => {
                  ...
                  <Row>
                    ...
                    <Col sm="8" md="auto">
                        <SubcategorySelector 
                          ...
                          onConfirm = {subCategories => {
                            this.setState({filters: {...filters, sub_categories: subCategories}})
                          }}
                        />
                    </Col>
                    ...
                    <Col sm="12" className="mt-1 primary">
                        ...
                        {filtersApplied.sub_categories && 
                          <>
                            <span>{filtersApplied.search && ', ' }Sub Categories: </span>
                            <span className="text-gray">
                            {
                              filtersApplied.sub_categories.map(
                                sc_id => this.state.operationalSubCategories.find(sc => sc.id === sc_id).name
                              ).limitedListFormat()
                            }
                            </span>
                          </>
