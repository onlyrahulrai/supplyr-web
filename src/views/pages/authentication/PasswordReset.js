...
export default function PasswordReset(props) {
    ...
    const onSubmit = (e) => {
        ...
        .catch(error => {
          let message = error.response?.data?.new_password2 ?? error.response?.data?.new_password1
          message = message ? message : error.response?.data?.token ? 'Link Expired' : error.message
          setError(message)
        })
        .finally(() => {
          setIsLoading(false)
